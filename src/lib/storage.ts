...
function store(): Storage {
  if (typeof window === 'undefined') return { getItem: () => null, setItem: () => {}, removeItem: () => {}, clear: () => {}, key: () => null, length: 0 }
  try {
...
export function isPrivateMode(): boolean {
  try { return !!sessionStorage.getItem(PRIVATE_FLAG) } catch { return false }
}

export function enablePrivateMode(): void {
  try { sessionStorage.setItem(PRIVATE_FLAG, '1') } catch {}
}

export function disablePrivateMode(): void {
  try { sessionStorage.removeItem(PRIVATE_FLAG) } catch {}
}
...
export function clearAllData(): void {
  ...
  const legacyKeys = ['cv-web-cvs', 'cv-web-current-cv', 'cv-web-style-overrides', 'cv-web-saves', 'cv-web-onboarded']
