      ...
      <div className="flex flex-col h-full">
          ...
          >
            {ed.hasSidebar ? (
              <div className="px-4 pb-3">
                ...
                <DndContext
                  ...
                  onDragEnd={ed.handleSidebarDragEnd}
                >
                  <SortableContext
                    items={(ed.layout.sidebarSections ?? []).map((s) => s.id)}
                ...
                {ed.availableSb.length > 0 && (
                  <select
                    ...
                    value=""
                    onChange={(e) => {
                      if (e.target.value) ed.addSidebarSection(e.target.value)
                    }}
                  >
                    <option value="">+ add to sidebar</option>
                    {ed.availableSb.map((id) => (
            ...
            <div className="px-4">
              <DndContext
                sensors={ed.sensors}
                collisionDetection={closestCenter}
                onDragEnd={ed.handleDragEnd}
              >
                <SortableContext
                  items={ed.layout.sections.map((s) => s.key)}
                  strategy={verticalListSortingStrategy}
                >
                  <div className="space-y-1.5">
                    {ed.layout.sections.map((item) => (
                      <SortableCard
                        ...
                        onToggleBreakable={() =>
                          ed.updateSection(item.key, (s) => ({ ...s, breakable: !s.breakable }))
                        }
              ...
              <div className="flex gap-1.5 pt-2">
                {ed.available.length > 0 && (
                  <select
                    ...
                    value=""
                    onChange={(e) => {
                      if (e.target.value) ed.addFullSection(e.target.value)
                    }}
                  >
                        ...
                        <div className="space-y-3 pb-3">
                          {ungrouped.map((p) => (
                            <StyleParamField
                              key={p.key}
                              p={p}
                              value={ed.style[p.key]}
                              onChange={ed.setStyleValue}
                            />
                          ))}
                        </div>
                      )}
        ...
        <div className="px-4 py-3 border-t border-gray-100 space-y-2 shrink-0">
          ...
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => ed.setShowSaveModal(true)}
            ...
            <button
              type="button"
              onClick={() => ed.importRef.current?.click()}
              className="flex-1 text-xs border border-gray-300 text-gray-700 py-1.5 rounded-lg hover:bg-gray-50 transition-colors"
            >
              Import ↑
            </button>
            <input
              ...
              accept=".json"
              className="hidden"
              onChange={ed.handleImport}
            />
          </div>
          <button
            type="button"
            onClick={ed.handleGenerate}
            disabled={ed.compileState !== 'idle'}
