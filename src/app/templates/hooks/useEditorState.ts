...
import type { LayoutStructure, EditorSection } from '../types'
import { parseLayoutStructure } from '../editor-utils'
...
export function useEditorState(initialLayout: Record<string, unknown>) {
  ...
  function updateSection(key: string, fn: (s: EditorSection) => EditorSection) {
    setLayout(prev => ({ ...prev, sections: prev.sections.map(s => s.key === key ? fn(s) : s) }))
  }
  ...
  function addFullSection(id: string) {
    setLayout(prev => ({ ...prev, sections: [...prev.sections, { kind: 'full', key: id, id, breakable: true }] }))
  }

  function addColumnsGroup() {
    const key = `columns-${Date.now()}`
    setLayout(prev => ({ ...prev, sections: [...prev.sections, { kind: 'columns', key, columns: 2, content: [[], []], breakable: true }] }))
  }
  ...
  function updateColumn(key: string, colIndex: number, cols: string[]) {
    updateSection(key, s =>
      s.kind !== 'columns' ? s : { ...s, content: s.content.map((c, i) => i === colIndex ? cols : c) }
    )
  }
  ...
  function addSidebarSection(id: string) {
    setLayout(prev => ({ ...prev, sidebarSections: [...prev.sidebarSections!, { id, breakable: true }] }))
  }

  function removeSidebarSection(id: string) {
    setLayout(prev => ({ ...prev, sidebarSections: prev.sidebarSections!.filter(s => s.id !== id) }))
  }

  function toggleSidebarBreakable(id: string) {
    setLayout(prev => ({
      ...prev,
      sidebarSections: prev.sidebarSections!.map(s => s.id === id ? { ...s, breakable: !s.breakable } : s),
    }))
  }
