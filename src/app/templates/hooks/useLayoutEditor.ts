...
import type { StyleParam, Panel } from '../types'
import type { SectionDef } from '../section-defs'
import { serializeLayout, serializeForTypst, usedIds } from '../editor-utils'
...
export function useLayoutEditor({
  ...
  const sidebarSet  = useMemo(() => new Set(editor.layout.sidebarSections?.map(s => s.id) ?? []), [editor.layout.sidebarSections])
  const usedInMain  = useMemo(() => usedIds(editor.layout.sections), [editor.layout.sections])
  const available   = useMemo(() => allMainIds.filter(id => !usedInMain.has(id) && !sidebarSet.has(id)), [allMainIds, usedInMain, sidebarSet])
  const availableSb = useMemo(() => allSidebarIds.filter(id => !sidebarSet.has(id)), [allSidebarIds, sidebarSet])
  ...
  return {
    ...
    // Derived
    hasSidebar, sidebarSet, available, availableSb, getLabel,
    // Compiler
