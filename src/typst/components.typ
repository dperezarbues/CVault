...
// ── Section anchor (queryable jump target) ────────────────────────────────────
// Labels inside place() or hide() cannot be used as link targets in Typst.
// metadata() has no visual output but IS in-flow and queryable.
#let section-anchor(id) = [#metadata(id)#label(id)]
...
// ── Page footer (page X / Y only) ────────────────────────────────────────────
#let cv-footer(_contact) = {
  set text(size: 5.5pt, fill: c-light)
  set par(justify: false, leading: 1em)
  pad(left: 0pt, right: margin-lg, y: 2pt)[
    #align(right)[
